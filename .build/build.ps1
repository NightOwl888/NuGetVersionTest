properties {
	...
	[string]$company_name = "NuGet Version Test"
	[string]$copyright = "Copyright © 2012 - $copyright_year $company_name"
}
...
task Restore -depends Clean -description "This task runs NuGet package restore" {
	exec { 
		&"$tools_directory\nuget\NuGet.exe" restore "$source_directory\NuGetVersionTest.sln"
	}
}

task Test -depends Clean, Init, Restore -description "This tasks runs the unit tests" {
	Write-Host "Unit tests disabled"
...
task Compile -depends Test -description "This task compiles the solution" {
	...
	exec { 
		msbuild "$source_directory\NuGetVersionTest\NuGetVersionTest.csproj" `
			/verbosity:quiet `
			/property:Configuration=$configuration `
			"/t:Clean;Rebuild" `
			/property:WarningLevel=3
...
task Package -depends Compile -description "This tasks makes creates the NuGet packages" {
	...
	$output_nuspec_file = "$template_directory\NuGetVersionTest.nuspec"

	exec { 
		&"$tools_directory\nuget\NuGet.exe" pack $output_nuspec_file -Version $packageVersion -OutputDirectory $output_directory -properties "copyright=$copyright"
	}
}
